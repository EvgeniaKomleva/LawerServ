...
class DefaultExpenseModel(models.Model):
    ...
    def analysis(queryset):
        total_value = 0 #queryset.aggregate(Sum('final_value'))['final_value__sum'] if queryset else 0
...
class Bill(DefaultExpenseModel):
    category = models.ForeignKey(BillCategory, null=True, on_delete=models.SET_NULL, related_name='Lis')
